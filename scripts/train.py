...
from homo_transformer.common import setup_config, setup_experiment, load_backbone
from homo_transformer.callbacks.gitdiff_callback import GitDiffCallback
from homo_transformer.callbacks.visualization_callback import VisualizationCallback
...
def maybe_resume_training(experiment):
    ...
    checkpoints = list(save_dir.glob(f'**/{experiment.resume_dir}/checkpoints/*.ckpt'))
...
def main(cfg):
    ...
    if ckpt_path is not None:
        model_module.backbone = load_backbone(ckpt_path, cfg)
    ...
    callbacks = [
        ...
        #ModelCheckpoint(filename='model-{epoch:02d}-{loss:.2f}', save_top_k=2, monitor="val_loss", mode="min"),
