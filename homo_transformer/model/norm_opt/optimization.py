...
class DampingNet(torch.nn.Module):
    ...
    def reset_damping(self):
        const = torch.zeros_like(self.const)
        self.register_parameter('const', torch.nn.Parameter(const))
...
def optimizer_step(g, H, lambda_=0, mute=False, mask=None, eps=1e-4):
...
def build_system(J, res, weight):
    grad = torch.einsum('...ndi,...nd->...ni', J, res)   # ... x N x 2
    grad = weight[..., None] * grad
    grad = grad.sum(-2)  # ... x 2

    Hess = torch.einsum('...ijk,...ijl->...ikl', J, J)  # ... x N x 2 x 2
    Hess = weight[..., None, None] * Hess
    Hess = Hess.sum(-3)  # ... x 2 x 2

    return grad, Hess

def calculate_delta(J, res, weight, damping):
    # solve the linear system
    g, H = build_system(J, res, weight)
    delta = optimizer_step(g, H, damping)
