...
class Decoder(nn.Module):
    def __init__(self, dim, blocks, residual=True, factor=2):
        ...
        for out_channels in blocks:
            layer = DecoderBlock(channels, out_channels, channels, residual, factor)
            layers.append(layer)

            channels = out_channels+dim
    ...
    def forward(self, x):
        ...
        y = None
        for feature, layer in zip(x[::-1], self.layers):
            if y == None:
                y = feature
            else:
                y = torch.cat([y,feature], dim=1)
            y = layer(y, y)
        return y
