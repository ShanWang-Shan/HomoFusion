...
class BaseViz:
    ...
    def visualize_pred(self, bev, pred, threshold=None):
        ...
        if pred.shape[2] > 1:
            #pred = np.amax(pred[:,:,1:], axis=-1) # 0 void
            pred = np.argmax(pred, axis=-1)/pred.shape[2]*0.8+0.2 # to 0.2~1 for visualization
            pred[pred<0.21] = 0
        else:
            pred = pred.squeeze(2)
        ...
        for b in range(min(batch_size, b_max)):
            if pred is not None:
                if 'bev' in pred.keys():
                    ...
                    pred_ = pred['mask']
                right = self.visualize_pred(gt[b], pred_[b].sigmoid())
            else:
                right = self.visualize_bev(gt[b])
